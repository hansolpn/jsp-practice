...
//		
//		// 응답하고자 하는 content의 타입과 문자열 인코딩을 설정
...
//                + "</html>";
//		
//		out.println(htmlCode); // 버퍼에 응답하고자 하는 데이터 작성하기
//		out.flush(); // 버퍼를 비우면서 작성한 내용을 브라우저로 밀어내기
//		out.close(); // 객체 해제
//	}
	
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// http 통신 중 get 요청이 발생했을때 자동으로 호출하는 메서드
		// 매개값으로 내장객체 request와 response가 전달되므로
		// 객체의 매서드를 통해 파라미터를 가져오거나, 페이지 이동이 가능합니다.
		
		System.out.println("doGet 매서드가 호출됨");
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// post방식으로 요청이 들어왔을 때, 함께 전송한 파라미터를 얻으실 때는
		// 인코딩 작업을 다시 거치서야 합니다.
		
		req.setCharacterEncoding("UTF-8");
		
		System.out.println("doPost 매서드가 호출됨");
	...
	@Override
	public void destroy() {
		// 서블릿 객체가 소멸할때 호출되는 메서드 (객체 소멸 시 1회 자동으로 호출)
		// 대부분 객체 반납이나 소멸 등에 사용
		System.out.println("destory 매서드가 호출됨");
	}
